...
def open_excel_file():
    try:
        os.startfile("swl_reports.xlsx")  # Works on linux
...
# Labels and input boxes for Callsign and QTH Locator
...
tk.Label(root, text="Name 1:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
...
tk.Label(root, text="QTH Locator 1:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
...
tk.Label(root, text="Callsign 2:").grid(row=0, column=2, padx=5, pady=5, sticky="e")
entry_callsign_2 = tk.Entry(root)
entry_callsign_2.grid(row=0, column=3, padx=5, pady=5)
entry_callsign_2.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Name 2:").grid(row=1, column=2, padx=5, pady=5, sticky="e")
entry_name_2 = tk.Entry(root)
entry_name_2.grid(row=1, column=3, padx=5, pady=5)
entry_name_2.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="QTH Locator 2:").grid(row=2, column=2, padx=5, pady=5, sticky="e")
entry_qth_locator_2 = tk.Entry(root)
entry_qth_locator_2.grid(row=2, column=3, padx=5, pady=5)
entry_qth_locator_2.bind("<Return>", next_field)  # Connect Enter

# Centering additional labels
tk.Label(root, text="Date (DD-MM-YYYY):").grid(row=6, column=1, padx=5, pady=5, sticky="e")
entry_date = tk.Entry(root)
entry_date.grid(row=6, column=2, padx=5, pady=5)
entry_date.insert(0, datetime.now().strftime("%d-%m-%Y"))
entry_date.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Time (UTC, HH:MM):").grid(row=7, column=1, padx=5, pady=5, sticky="e")
entry_time = tk.Entry(root)
entry_time.grid(row=7, column=2, padx=5, pady=5)
entry_time.insert(0, datetime.now().strftime("%H:%M"))
entry_time.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Frequency (MHz):").grid(row=8, column=1, padx=5, pady=5, sticky="e")
entry_frequency = tk.Entry(root)
entry_frequency.grid(row=8, column=2, padx=5, pady=5)
entry_frequency.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Mode:").grid(row=9, column=1, padx=5, pady=5, sticky="e")
entry_mode = tk.StringVar(root)
mode_menu = tk.OptionMenu(root, entry_mode, "SSB", "CW", "AM", "FM", "Digital")  # Different modi
mode_menu.grid(row=9, column=2, padx=5, pady=5)

tk.Label(root, text="Readability:").grid(row=10, column=1, padx=5, pady=5, sticky="e")
entry_readability = tk.Entry(root)
entry_readability.grid(row=10, column=2, padx=5, pady=5)
entry_readability.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Signal strength:").grid(row=11, column=1, padx=5, pady=5, sticky="e")
entry_signalstrength = tk.Entry(root)
entry_signalstrength.grid(row=11, column=2, padx=5, pady=5)
entry_signalstrength.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Tone:").grid(row=12, column=1, padx=5, pady=5, sticky="e")
entry_tone = tk.Entry(root)
entry_tone.grid(row=12, column=2, padx=5, pady=5)
entry_tone.bind("<Return>", next_field)  # Connect Enter

tk.Label(root, text="Details:").grid(row=13, column=1, padx=5, pady=5, sticky="e")
entry_details = tk.Entry(root)
entry_details.grid(row=13, column=2, padx=5, pady=5)
entry_details.bind("<Return>", next_field)  # Connect Enter

# Listbox for reports
report_list = tk.Listbox(root, height=10, width=70)
report_list.grid(row=14, column=1, columnspan=2, padx=10, pady=5)

# Save button
save_button = tk.Button(root, text="Save", command=save_swl_report)
save_button.grid(row=15, column=1, padx=5, pady=5, sticky="ew")

# Open button
open_button = tk.Button(root, text="Open logfile", command=open_excel_file)
open_button.grid(row=15, column=2, padx=5, pady=5, sticky="ew")

# About button
about_button = tk.Button(root, text="About", command=show_about)
about_button.grid(row=16, column=1, padx=5, pady=5, sticky="ew")

# Exit button
exit_button = tk.Button(root, text="Exit", command=root.quit)
exit_button.grid(row=16, column=2, padx=5, pady=5, sticky="ew")
